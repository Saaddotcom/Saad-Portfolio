...
export default function GooeyNav({
  ...
  const makeParticles = (element) => {
    ...
    for (let i = 0; i < particleCount; i += 1) {
      ...
      setTimeout(() => {
        ...
        particle.style.setProperty('--color', `var(--gooey-color-${p.color}, #fff)`)
  ...
  return (
    <div className="gooey-nav-container" ref={containerRef}>
          ...
          {items.map((item, index) => (
            <li key={item.label} className={activeIndex === index ? 'active' : ''}>
              <a
                ...
                onKeyDown={(event) => {
                  if (event.key === 'Enter' || event.key === ' ') handleClick(event, item, index)
                }}
              >
                {item.label}
              </a>
      ...
      <span className="effect text" ref={textRef} style={{ color: '#FFFFFF' }} />
    </div>
  )
}
